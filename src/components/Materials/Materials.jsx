...
const Materials = () => {
  ...
  const handleDownload = (material) => {
    // إذا كان الملف محملاً من الجهاز (يحتوي على fileData)
    if (material.fileData) {
      ...
      // محاولة التحميل من الرابط الافتراضي
  ...
  const getIconComponent = (iconName) => {
    ...
    return icons[iconName] || FaFilePdf;
  };
          ...
          <div className="flex flex-col md:flex-row gap-4">
            <div className="flex-1 relative">
              <input
                ...
                className={`w-full px-5 py-3 pr-12 rounded-xl border focus:border-gold/30 transition-all duration-300 text-theme-primary placeholder-textMuted outline-none ${
                  isDark 
                    ? 'glass-premium border-white/5' 
                    : 'bg-white border-gold/20 shadow-sm'
                }`}
              />
              <FaSearch className={`absolute right-4 top-1/2 transform -translate-y-1/2 ${isDark ? 'text-textMuted' : 'text-theme-muted'}`} />
            </div>
          ...
          {filteredMaterials.map((material, index) => {
            ...
            return (
              ...
              >
                ...
                <div className={`relative p-5 rounded-2xl border ${material.borderColor || 'border-gold/20'} transition-all duration-500 overflow-hidden ${
                  isDark 
                    ? 'glass-premium' 
                    : 'bg-white border-gold/20 shadow-md hover:shadow-xl'
                }`}>
                  <div className={`absolute inset-0 bg-gradient-to-br ${material.color || 'from-gold/5 to-transparent'} opacity-0 group-hover:opacity-100 transition-all duration-700`} />
                  
                  <div className="relative flex items-center justify-between">
                    <div className="flex items-center gap-4 flex-1">
                      <div className={`w-14 h-14 rounded-xl bg-gradient-to-br ${material.color || 'from-gold/5 to-transparent'} flex items-center justify-center group-hover:scale-110 transition-all duration-500`}>
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="flex flex-wrap items-center gap-3 mt-1">
                          ...
                          <span className="text-xs text-theme-muted flex items-center gap-1">
                            ...
                            {material.downloads || 0} تحميل
                          </span>
                          {material.fileData && (
                            <span className="text-xs text-green-500">📁 محلي</span>
                          )}
                        </div>
                      </div>
                    </div>
                  ...
                  <div className={`absolute top-3 right-3 px-2 py-1 rounded-lg glass-premium text-[10px] text-gold border ${material.borderColor || 'border-gold/20'} opacity-0 group-hover:opacity-100 transition-all duration-500`}>
                    {material.category}
                  </div>
